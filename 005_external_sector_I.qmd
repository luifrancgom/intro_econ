...
- In the case of Colombia you can find the balance of payments in
    ...
    + [https://www.banrep.gov.co/](https://www.banrep.gov.co/) > Estadísticas > Tasas de cambio, sector externo y derivados > Sector Externo > Balanza de pagos
...
```{r include=FALSE}
# For more information check out https://www.imf.org/en/home > DATA >
# International Financial Statistics > Query > Indicator >
# Balance of Payments, Capital Account, Debit [BPM6], US Dollar
# Also using the new syntax package use:
## data_bases <- imf_databases(times = 1) %>% as_tibble()
## parameters <- imf_parameters(database_id = "BOP", times = 1)
## parameters %>% .$indicator %>% as_tibble()
### Also check out the publication: 
### BALANCE OF PAYMENTS STATISTICS: Introductory Notes
