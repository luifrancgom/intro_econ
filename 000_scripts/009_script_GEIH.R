...
folder_path <- "000_data/009_geih/nov_2025"
...
##### problems(ocupados)
folder_path_ocupados <- str_c(folder_path, "ocupados.csv", sep = "/")
ocupados <- read_delim(
  file = folder_path_ocupados,
  delim = ";",
  # Check the columns in case of any problem
  col_types = cols(
...
folder_path_caracteristicas_generales_seguridad_social_en_salud_y_educacion <- str_c(
  folder_path,
  "caracteristicas_generales_seguridad_social_en_salud_y_educacion.csv",
  sep = "/"
)
caracteristicas_generales_seguridad_social_en_salud_y_educacion <- read_delim(
  file = folder_path_caracteristicas_generales_seguridad_social_en_salud_y_educacion,
  delim = ";",
  locale = locale(decimal_mark = ".")
)
...
ingreso_nivel_educativo <- ocupados_ingreso_laboral |>
  left_join(
    y = nivel_educativo_caract_indiv,
    by = join_by(MES, PER, PERIODO, DIRECTORIO, SECUENCIA_P, ORDEN, HOGAR)
  )
...
group_ingreso_nivel_educativo_categories <- ingreso_nivel_educativo_categories |>
  select(nivel_educativo, nivel_ingreso, FEX_C18) |>
  group_by(nivel_educativo, nivel_ingreso) |>
  summarise(FEX_C18 = sum(FEX_C18), .groups = "drop")
