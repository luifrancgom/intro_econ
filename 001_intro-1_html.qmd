...
pop_pyramid_tbl_tidy <- pop_pyramid_tbl |>
  ...
  mutate(
    genero_en = ifelse(test = genero == "hombres",
      yes = "Men",
      no = "Women"
    ),
    .after = genero
  ...
  mutate(
    pct_pop_gen = ifelse(test = genero_en == "Men",
      yes = pct_pop,
      no = -pct_pop
    ),
    .after = pct_pop
...
posicion_ideologica_clean <- posicion_ideologica |>
  ...
  mutate(p5328_label = fct_relevel(
    .f = p5328_label,
    "Izquierda",
    "Centro izquierda",
    "Centro",
...
household_income_nper_tbl <- household_tbl_tidy |>
  ...
  mutate(
    pct_fex_c_text = number(pct_fex_c,
      suffix = "%",
      scale = 100,
      accuracy = 0.01
    ),
