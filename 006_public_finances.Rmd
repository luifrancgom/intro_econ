...
- This presentation is based on [@cardenas_santamaria_introduccion_2013, Chapter 6]
...
```{r}
cng_fb <- read_excel(path = "000_data/006_balance_gobierno_nacional_central_1994-2020.xlsx", 
           sheet = 2, 
           range = "B5:AC100") %>%
  rename(Concepto = "CONCEPTO") %>% 
  slice(-c(1:2,4,33,35,50,52,54,71,73,75,77,79,81,83,94)) %>% 
  select(Concepto, `2020`)
...
cng_fb_table_footnote %>% 
  ...
  footnote(general       = c('Source: Ministerio de Hacienda y Crédito Público - Balance Gobierno Nacional Central', 'Last update: 2021-06-18'),
           general_title = "",
...
cng_fb <- read_excel(path = "000_data/006_balance_gobierno_nacional_central_1994-2020.xlsx", 
  ...
  mutate(`2020` = (`2020`*10^9) %>% scales::number(big.mark = ",", 
                                                   accuracy = 0.1))
...
```{r out.width="90%"}
pgn_2020_what_spent <- read_excel("000_data/006_en_que_se_gasta_PTE.xlsx",
           sheet = 1, 
           range = "E1:M5") %>%
...
pgn_2020_what_spent %>% 
  slice(1:3) %>% 
  mutate(png_pct = ApropiacionInicial / pgn_2020_what_spent$ApropiacionInicial[4],
  ...
  scale_fill_tq() +
  labs(fill = NULL,
       title = str_glue("What is the nation's general budget spent on in 2020?"),
       subtitle = str_glue("Total national budget: {scales::number(pgn_2020_what_spent$ApropiacionInicial[4], big.mark = ',')} COP
                           Funcionamiento: {scales::number(pgn_2020_what_spent$ApropiacionInicial[1], big.mark = ',')} COP
                           Servicio De La Deuda: {scales::number(pgn_2020_what_spent$ApropiacionInicial[2], big.mark = ',')} COP,
                           Inversión: {scales::number(pgn_2020_what_spent$ApropiacionInicial[3], big.mark = ',')} COP"),
...
```{r}
pgn_2020_who_spends <- read_excel("000_data/006_quien_gasta_PTE.xlsx", 
                                  sheet = 1, 
                                  range = "A1:L196") %>%
  select(CodigoSector, NombreSector, ApropiacionInicial) %>% 
  slice(-1)
  
pgn_2020_who_spends_table <- pgn_2020_who_spends %>% 
...
```{r out.width="90%"}
pgn_2020_who_spends_table %>% 
    mutate(NombreSector = case_when(
      ApropiacionInicial < pgn_2020_who_spends_table$ApropiacionInicial[11] ~ "Otros Sectores",
      TRUE ~ NombreSector)) %>% 
  ...
  scale_fill_tq() +
  labs(fill = NULL,
       title = str_glue("Who spends nation's general budget spent on in 2020?"),
       subtitle = str_glue("Total national budget: {scales::number(pgn_2020_what_spent$ApropiacionInicial[4], big.mark = ',')} COP
