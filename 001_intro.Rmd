...
# Proyecciones y retroproyecciones de población nacional para el periodo 1950-2017 y 2018-2070 con base en el CNPV 2018 >
# Serie nacional de población por área, sexo y edad para el periodo 2018 – 2070
pop_pyramid_tbl <- readxl::read_excel(path = "000_data/001_anexo_proyecciones_poblacion_Nacional2018_2070.xlsx", 
                                      sheet = 1, 
                                      range = "C12:KY171") %>% 
  janitor::clean_names() %>% 
  filter(area_geografica == "TOTAL") %>% 
  select(-c(area_geografica, total_general, total_hombres, total_mujeres)) %>% 
...
pop_pyramid_tbl %>% 
  ...
  labs(x = NULL,
       y = NULL,
       fill = NULL, 
       caption = str_glue('Source: DANE - Proyecciones y retroproyecciones de población nacional 2018 - 2070 (área, sexo y edad)
...
- **ingtotug**: "Ingreso total de la unidad de gasto **antes** de imputación de arriendo a propietarios y usufructuarios" [@dnp_pobreza_2012, p 16-17]
...
- **ingtotugarr**: "Ingreso total de la unidad de gasto con imputación de arriendo a propietarios y usufructuarios" [@dnp_pobreza_2012, p 24-26]
...
# Medición de Pobreza Monetaria y Desigualdad > Hogares
household_tbl <- read_delim(file = "000_data/001_hogares_pobreza_monetaria_desigualdad_2020.csv", 
                                   delim = ";", 
...
household_income_nper_tbl %>% 
  ...
  labs(x = NULL,
       ...
       caption = str_glue('Source: DANE - Medición de Pobreza Monetaria y Desigualdad 2020
...
- If you want to explore more about this topic from around the world in a visual way check out[^2]:
    ...
    + https://youtu.be/u4L130DkdOw (Configure spanish subtitles in the setting options)
...
- Housing (vivienda) ownership  status

    + "Propia, totalmente pagada"
    + "Propia, la están pagando"
    + "En arriendo o subarriendo"
    + "En usufructo"
    + "Posesión sin titulo (ocupante)"
    + "Otra"
...
```{r out.width = "85%"}
schools_tbl <- read_delim(file = "000_data/001_caratula_unica_sede_educativa.csv", 
                                   delim = ";", 
                                   locale = locale(decimal_mark = ","))
...
schools_bilin_tbl <- schools_tbl %>% 
  ...
  mutate(pct_n = n / nrow(schools_tbl),
         ...
         TIPOBIL_NOMBRE = case_when(
           ...
           is.na(TIPOBIL_NOMBRE) ~ "Sin información",
           TRUE ~ TIPOBIL_NOMBRE ),
...
schools_bilin_tbl %>% 
  ...
  labs(x = NULL,
       y = NULL,
       title = "Bilingualism level in the Colombian school education system",
       subtitle = str_glue("Year: 2020 
                           Number of educational establishments: {scales::number(sum(schools_bilin_tbl$n), big.mark = ',')} 
                           Number of not official educational establishments: {scales::number(sum(schools_bilin_tbl$n[1:4]), big.mark = ',')}
                           Number of official educational establishments: {scales::number(sum(schools_bilin_tbl$n[5:8]), big.mark = ',')}"),
       caption = str_glue('Source: DANE - Educación Formal - 2020 - Carátula única sede educativa
...
manufac_tbl <- read_delim(file = "000_data/001_encuesta_anual_manufacturera_2020.csv", 
                            delim = ",", 
                            locale = locale(decimal_mark = ",")) %>% 
  janitor::clean_names() %>%
...
egg_jan_2022_tbl <- read_excel(path = "000_data/001_series_historicas_precios_mayoristas_2022_may.xlsx", 
                           sheet = 6, 
                           range = "A7:E4489") %>% 
  janitor::clean_names() %>%
  select(fecha, producto:precio_promedio_por_kilogramo) %>%
...
egg_jan_jan_tbl <- egg_jan_2021_tbl %>% 
  bind_rows(egg_jan_2022_tbl) %>%
...
- Example "Declaración de Renta Iván Duque Márquez Año Gravable 2019":
...
```{r out.width="60%"}
knitr::include_graphics("000_data/001_patrimonio_duque.png")
```
...
```{r out.width="60%"}
knitr::include_graphics("000_data/001_rentas_trabajo_duque.png")
```
