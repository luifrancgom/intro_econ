...
# royecciones y retroproyecciones de población nacional para el periodo 1950-2017 y 2018-2070 con base en el CNPV 2018 >
# Serie nacional de población por área, sexo y edad para el periodo 2018-2070

# Import
# Import
pop_pyramid_tbl <- read_excel(
  path = "000_data/001_serie_nacional_de_poblacion_por_area_sexo_y_edad_para_el_periodo_2020_2070.xlsx",
  sheet = 3,
  range = "B9:KW128"
)
...
pop_pyramid_tbl_tidy <- pop_pyramid_tbl |>
  clean_names() |>
  dplyr::rename(ano = x1, area_geografica = x2) |>
  ...
  select(ano:mujeres_100_anos_y_mas) |>
  rename_with(.fn = \(x) {
    str_replace(
      string = x,
      pattern = r"(^([a-z]+_[0-9]+)_.+$)",
      replacement = r"(\1)"
    )
  }) |>
  pivot_longer(
    cols = hombres_0:mujeres_100,
  ...
  mutate(
    genero_en = ifelse(test = genero == "hombres", yes = "Men", no = "Women"),
    .after = genero
  ) |>
  mutate(
    pct_pop_gen = ifelse(
...
#| fig-cap: Population pyramids in Colombia[^1]
#| fig-height: 6
...
# Import
height_adults_col <- read_csv(
  file = "000_data/001_average_height_by_year_of_birth/average_height_by_year_of_birth.csv"
)
...
height_adults_col_tidy <- height_adults_col |>
  ...
  mutate(
    sex = fct_reorder(.f = sex, .x = mean_height) |>
      fct_rev()
  )

## Auxiliary variables
last_update_height_adults_col <- ymd("2019-05-14")
```
...
height_adults_col_tidy |>
  ...
  scale_y_continuous(
    breaks = seq(
      round(min(height_adults_col_tidy$mean_height)),
  ...
  labs(
    ...
    color = NULL,
    subtitle = str_glue(
      "Period: {min(height_adults_col_tidy$year)} - {max(height_adults_col_tidy$year)}"
    ),
    caption = str_glue(
      "Source: NCD RisC and Our Word In Data - Human Height
                          Last update: {last_update_height_adults_col}"
    )
  ) +
  theme(
...
elecciones_partidos <- read_csv(
  file = "000_data/001_elecciones_y_partidos_2023.csv"
)
...
## Auxiliary variables
last_update_posicion_ideologica <- ymd("2024-04-15")
year_posicion_ideologica <- 2023
```
...
posicion_ideologica_clean |>
  ...
  geom_segment(aes(xend = 0, yend = p5328_label), color = umng_palette[1]) +
  geom_point(
    shape = 21,
    fill = umng_palette[2],
    size = 3
  ) +
  geom_text(aes(label = pct_n_label), hjust = -0.5, size = 5) +
  scale_x_continuous(
    limits = c(
      0,
  ...
  labs(
    ...
    subtitle = str_glue(
      "Year: {year_posicion_ideologica}
                           ...
                           Original question: Las personas cuando piensan en política utilizan los términos izquierda y derecha. 
                           En una escala de 1 a 10 donde 1 significa izquierda y 10 significa derecha 
                           ¿dónde se ubicaría usted?"
    ),
    caption = str_glue(
      "Source: DANE - Encuesta de Cultura Política - ECP - {year_posicion_ideologica}
                          Last update date: {last_update_posicion_ideologica}"
    )
  ) +
  theme(
    panel.border = element_rect(
...
# Medición de Pobreza Monetaria y Desigualdad 2024 > 
# BDatos-Pobreza_Monetaria_Desigualdad-2024 >
# Hogares >
# Hogares.csv

# Import
household_tbl <- read_delim(
  file = "000_data/001_hogares_pobreza_monetaria_desigualdad_2024.csv",
  delim = ",",
...
ingtotug_zero <- household_tbl_tidy |> 
  filter(ingtotug == 0) |>
  _$fex_c |>
  sum() |>
  floor()
...
## Total households
total_fex_c <- summarize(household_tbl_tidy, sum_fex_c = sum(fex_c))$sum_fex_c
...
household_income_nper_tbl <- household_tbl_tidy |>
  ...
  mutate(
    range_ingtotug = cut(
      ...
      breaks = c(
        seq.int(
          from = min(household_tbl_tidy$ingtotug),
          by = 500000,
          length.out = 20
        ),
  ...
  summarize(
    pct_fex_c = sum(pct_fex_c),
    mean_nper = mean(nper),
    .groups = "drop"
  ) |>
  mutate(
    pct_fex_c_text = number(
      pct_fex_c,
      suffix = "%",
      scale = 100,
      accuracy = 0.01
    ),
    mean_nper_text = number(mean_nper, accuracy = 0.01)
  )

# Helpers
last_update_household <- ymd("2026-01-26")
year_household <- 2024
```
...
household_income_nper_tbl |>
  ...
  geom_segment(
    aes(xend = 0, yend = fct_rev(range_ingtotug)),
  ...
  geom_text(aes(label = pct_fex_c_text), hjust = -0.5, size = 4) +
  scale_x_continuous(
    limits = c(
      0,
  ...
  labs(
    x = NULL,
    y = str_glue("Monthly mean income range ({year_household} COP)"),
    subtitle = str_glue(
      "Year: 2024
    ...
    caption = str_glue(
      "Source: DANE - Medición de Pobreza Monetaria y Desigualdad {year_household}
                          Last update date: {last_update_household}"
    )
  ) +
  theme(
    panel.border = element_rect(
...
-   If you want to explore more about this topic using data from the year 2023 check out[^2]:

    -   <https://www.ofiscal.org/> \> Interactúa \> Calcule dónde se ubica su hogar según su ingreso

-   If you want to explore more about this topic from around the world in a visual way check out[^3]:
    ...
    -   <https://www.gapminder.org> \> Resources \> See the reality behind the data


        -   <https://www.gapminder.org/dollar-street>

[^2]: Observatorio Fiscal de la Pontifica Universidad Javeriana
...
household_income_nper_tbl |>
  ...
  geom_point(shape = 21, fill = umng_palette[2], size = 3) +
  geom_segment(
    aes(xend = 0, yend = fct_rev(range_ingtotug)),
    color = umng_palette[1]
  ) +
  geom_text(aes(label = mean_nper_text), hjust = -0.5, size = 4) +
...
-   Housing (vivienda) ownership status[^4]
...
[^4]: For a detail definition of these categories checkout [@dane_manual_2022, p. 35]
...
household_housing_tbl |>
  ...
  geom_segment(aes(xend = 0, yend = p5090), color = umng_palette[1]) +
  geom_point(
    shape = 21,
    fill = umng_palette[2],
    size = 3
  ) +
  geom_text(aes(label = pct_fex_c_text), hjust = -0.5, size = 5) +
  scale_x_continuous(
    limits = c(
      0,
...
# Helpers
last_update_schools <- ymd("2024-06-28")
year_update_schools <- 2023

# Import
schools_tbl <- read_csv(
  file = "000_data/001_caratula_unica_de_la_sede_educativa_2023.csv"
)
...
schools_bilin_tbl <- schools_tbl |>
  ...
  mutate(
    pct_n = n / nrow(schools_tbl),
    pct_n_text = number(pct_n, suffix = "%", scale = 100, accuracy = 0.01),
    TIPOBIL_NOMBRE = case_when(
      ES_BILINGUE == "NO" ~ "No bilingüe",
      is.na(ES_BILINGUE) == TRUE ~ "Sin información",
      TIPOBIL_NOMBRE ==
        "Bilingüismo como profundización y énfasis en el plan de estudios" ~ "Profundización/énfasis",
...
schools_bilin_tbl |>
  ...
  geom_text(aes(label = pct_n_text), vjust = -0.8, size = 5) +
  scale_y_continuous(
    limits = c(
      0,
  ...
  labs(
    ...
    y = NULL,
    subtitle = str_glue(
      "Year: {year_update_schools}
                           Total educational establishments: {number(sum(schools_bilin_tbl$n), big.mark = ',')}
                           Total non official educational establishments: {number(sum(schools_bilin_tbl$n[1:4]), big.mark = ',')}
                           Total official educational establishments: {number(sum(schools_bilin_tbl$n[5:8]), big.mark = ',')}"
    ),
    caption = str_glue(
      "Source: DANE - Educación Formal - {year_update_schools} - Carátula única sede educativa
                          Última fecha de actualización: {last_update_schools}"
    )
  ) +
  theme(
    panel.border = element_rect(
...
-   To the [**R Core Team**](https://www.r-project.org/contributors.html), the creators of [**RStudio IDE**](https://posit.co/download/rstudio-desktop/), [**Positron**](https://positron.posit.co/), [**Quarto**](https://quarto.org/) and the authors and maintainers of the packages [**tidyverse**](https://CRAN.R-project.org/package=tidyverse), [**readxl**](https://CRAN.R-project.org/package=readxl), [**janitor**](https://CRAN.R-project.org/package=janitor), [**scales**](https://CRAN.R-project.org/package=scales), [**plotly**](https://CRAN.R-project.org/package=plotly) and [**tinytex**](https://CRAN.R-project.org/package=tinytex) for allowing me to access these tools without paying for a license
