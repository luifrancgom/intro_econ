...
# Import
homicide <- wb_data( # Intentional homicides (per 100,000 people)
...
homicide_pivot_wider |>
  ...
  labs(
    x = str_glue("Intentional homicides (per 100,000 people) in {initial_year}"),
...
homicide_female_pivot_wider |> 
  ...
  geom_label_repel(data = homicide_female_pivot_wider |> 
                   ...
                   box.padding = 1) + 
  labs(
       x = str_glue("Intentional homicides female (per 100,000 people) in {initial_year}"),
       y = str_glue("Intentional homicides female (per 100,000 people) in {last_year}"),
       fill = NULL, 
       title = str_glue("Period: {initial_year} vs {last_year}"),
...
homicide_male_pivot_wider |> 
  ...
  geom_label_repel(data = homicide_male_pivot_wider |> 
                   ...
                   box.padding = 1) +
  labs(
       x = str_glue("Intentional homicides male (per 100,000 people) in {initial_year}"),
       y = str_glue("Intentional homicides male (per 100,000 people) in {last_year}"),
       fill = NULL, 
       title = str_glue("Period: {initial_year} vs {last_year}"),
...
# Helpers
last_update_pol_nal <- ymd("2025-05-05")
```
...
crime_col |> 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black") + 
  scale_fill_gradient(low = umng_palette[2], 
                      high = umng_palette[1]) +
  labs(fill = NULL,
