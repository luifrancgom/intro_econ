...
library(wbstats)
library(scales)
# Take into account key api in .Renviron
## https://docs.ropensci.org/comtradr/articles/comtradr.html#authentication
library(comtradr)
```
...
trade_col_clean |>
  ggplot() +
  geom_point(
    aes(x = date, y = value, fill = indicator_id),
    shape = 21,
    color = "black",
    show.legend = FALSE
  ) +
  ...
  labs(
    ...
    color = "",
    title = str_glue(
      "Period: {min(trade_col_clean$date)}-{max(trade_col_clean$date)}"
    ),
    subtitle = str_glue(
      "Variables Units: constant Local Currency Units Base Year 2015
...
![Colombian good exports **HS 4 digits (HS92) edition** year 2024 [@simoes_economic_2011]](000_data/005_col_exports_2024_hs4_92.png){.nostretch #fig-colombian-goods-exports-2024 width=100% fig-align="center"}
...
![Colombian goods imports **HS 4 digits (HS92) edition** year 2024 [@simoes_economic_2011]](000_data/005_col_imports_2024_hs4_92.png){.nostretch #fig-colombian-goods-imports-2024 width=100% fig-align="center"}
...
first_year <- 2000
last_year <- 2023
last_update_trade_services <- ymd("2026-02-05")
```

```{r}
# Import
trade_services_export_first <- ct_get_data(
  type = "services",
  commodity_classification = "EB02",
  commodity_code = c(
    # 1. Transportation
    "205",
    # 2. Travel
    ...
    "268",
    # 10. Personal, cultural, and recreational service
    "287",
...
# Tidy
trade_services_export_first_top_8_clean <- trade_services_export_first |>
  as_tibble() |>
  select(
    ref_year,
    reporter_desc,
  ...
  mutate(
    cmd_desc = str_replace_all(
      string = cmd_desc,
      pattern = r"([0-9]+\. )",
      replacement = ""
    )
  ) |>
  slice_max(order_by = cifvalue, n = 8) |>
  mutate(cmd_desc = fct_reorder(.f = cmd_desc, .x = cifvalue)) |>
  mutate(
    cifvalue_label = number(
      x = cifvalue,
      scale = 1e-9,
      suffix = "B",
      accuracy = 0.01
...
# Visualization
trade_services_export_first_top_8_clean |>
  mutate(cmd_desc = fct_reorder(.f = cmd_desc, .x = cifvalue)) |>
  ggplot(aes(
    cifvalue,
    cmd_desc
  )) +
  geom_segment(aes(xend = 0, yend = cmd_desc), color = umng_palette[1]) +
  geom_point(
    shape = 21,
    fill = umng_palette[2],
    size = 3
  ) +
  geom_text(aes(label = cifvalue_label), hjust = -0.5, size = 5) +
  scale_x_continuous(
    limits = c(
      0,
      max(trade_services_export_first_top_8_clean$cifvalue) + 2e9
    )
  ) +
  labs(
    x = NULL,
    ...
    caption = str_glue(
      "Source: UN Comtrade Database - Trade Services
...
```{r}
# Import
trade_services_export_last <- ct_get_data(
  type = "services",
  commodity_classification = "EB02",
  commodity_code = c(
    # 1. Transportation
...
# Tidy
trade_services_export_last_top_8_clean <- trade_services_export_last |>
  as_tibble() |>
  select(
    ref_year,
    reporter_desc,
...
# Visualization
trade_services_export_last_top_8_clean |>
  mutate(cmd_desc = fct_reorder(.f = cmd_desc, .x = cifvalue)) |>
  ggplot(aes(
    cifvalue,
    cmd_desc
  ...
  scale_x_continuous(
    limits = c(
      0,
      max(trade_services_export_last_top_8_clean$cifvalue) + 2e9
    )
  ) +
  labs(
    x = NULL,
...
```{r}
# Import
trade_services_import_first <- ct_get_data(
  type = "services",
  commodity_classification = "EB02",
  commodity_code = c(
    # 1. Transportation
...
# Tidy
trade_services_import_first_top_8_clean <- trade_services_import_first |>
  as_tibble() |>
  select(
    ref_year,
    reporter_desc,
...
# Visualization
trade_services_import_first_top_8_clean |>
  mutate(cmd_desc = fct_reorder(.f = cmd_desc, .x = cifvalue)) |>
  ggplot(aes(
    cifvalue,
    cmd_desc
  ...
  scale_x_continuous(
    limits = c(
      0,
      max(trade_services_import_first_top_8_clean$cifvalue) + 2e9
    )
  ) +
  labs(
    x = NULL,
...
```{r}
# Import
trade_services_import_last <- ct_get_data(
  type = "services",
  commodity_classification = "EB02",
  commodity_code = c(
    # 1. Transportation
...
# Tidy
trade_services_import_last_top_8_clean <- trade_services_import_last |>
  as_tibble() |>
  select(
    ref_year,
    reporter_desc,
...
# Visualization
trade_services_import_last_top_8_clean |>
  mutate(cmd_desc = fct_reorder(.f = cmd_desc, .x = cifvalue)) |>
  ggplot(aes(
    cifvalue,
    cmd_desc
  ...
  scale_x_continuous(
    limits = c(
      0,
      max(trade_services_import_last_top_8_clean$cifvalue) + 2e9
    )
  ) +
  labs(
    x = NULL,
...
[**scales**](https://CRAN.R-project.org/package=scales), [**comtradr**](https://CRAN.R-project.org/package=comtradr) and [**tinytex**](https://CRAN.R-project.org/package=tinytex) for allowing me to access these tools without paying for a license
