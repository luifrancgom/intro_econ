...
```{r out.width="90%"}
homicide_1990_2018 <- wbstats::wb_data(# Intentional homicides (per 100,000 people)
  indicator = "VC.IHR.PSRC.P5", 
  start_date = 1990, 
  end_date = 2018) %>% 
  filter(!is.na(VC.IHR.PSRC.P5), date %in% c(1990, 2018)) %>% 
...
homicide_1990_2018 %>% 
  ...
  scale_fill_tq() +
  labs(fill = NULL, 
       title = str_glue("Intentional homicides (per 100,000 people): 1990 vs 2018"),
...
```{r out.width="90%"}
homicide_female_1990_2018 <- wbstats::wb_data(# Intentional homicides, female (per 100,000 female)
  indicator = "VC.IHR.PSRC.FE.P5", 
  start_date = 1990, 
  end_date = 2018) %>% 
  filter(!is.na(VC.IHR.PSRC.FE.P5), date %in% c(1990, 2018)) %>% 
  select(iso2c, iso3c, country, date, VC.IHR.PSRC.FE.P5) %>% 
  pivot_wider(names_from = date, values_from = VC.IHR.PSRC.FE.P5) %>% 
  drop_na()

homicide_female_1990_2018 %>% 
  ggplot(aes(x = `1990`, y = `2018`)) + 
  geom_abline(slope = 1, color = 'black') +
  geom_point(shape = 21, 
             size = 4,
             fill = "red") + 
  geom_label_repel(data = homicide_female_1990_2018 %>% 
                     filter(country == "Colombia"),
                   aes(label = country), 
                   seed = 1234,                    
                   box.padding = 1) + 
  scale_fill_tq() +
  labs(fill = NULL, 
       title = "# Intentional homicides, female (per 100,000 female): 1990 vs 2018",
       subtitle = str_glue("Number of countries: {nrow(homicide_female_1990_2018)}"),
       caption = str_glue("Source: World Development Indicators - World Bank
                          Last update: 2022-06-14")) + 
  theme(panel.border    = element_rect(fill = NA, color = "black"),
        plot.background   = element_rect(fill = "#f3fcfc"),
...
```{r out.width="90%"}
homicide_male_1990_2018 <- wbstats::wb_data(# Intentional homicides, male (per 100,000 male)
  indicator = "VC.IHR.PSRC.MA.P5", 
  start_date = 1990, 
  end_date = 2018) %>% 
  filter(!is.na(VC.IHR.PSRC.MA.P5), date %in% c(1990, 2018)) %>% 
  select(iso2c, iso3c, country, date, VC.IHR.PSRC.MA.P5) %>% 
  pivot_wider(names_from = date, values_from = VC.IHR.PSRC.MA.P5) %>% 
  drop_na()

homicide_male_1990_2018 %>% 
  ggplot(aes(x = `1990`, y = `2018`)) + 
  geom_abline(slope = 1, color = 'black') +
  geom_point(shape = 21, 
             size = 4,
             fill = "red") + 
  geom_label_repel(data = homicide_male_1990_2018 %>% 
                     filter(country == "Colombia"),
                   aes(label = country), 
                   seed = 1234,                    
                   box.padding = 1) + 
  scale_fill_tq() +
  labs(fill = NULL, 
       title = "# Intentional homicides, male (per 100,000 male): 1990 vs 2018",
       subtitle = str_glue("Number of countries: {nrow(homicide_female_1990_2018)}"),
       caption = str_glue("Source: World Development Indicators - World Bank
                          Last update: 2022-06-14")) + 
  theme(panel.border    = element_rect(fill = NA, color = "black"),
