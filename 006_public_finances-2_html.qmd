...
# Helpers
last_update_tax_revenue <- ymd("2025-05-27")

# Import
pgn <- read_csv(file = "000_data/006_informacion_de_gastos_del_presupuesto_general_de_la_nacion_2025-07-25.zip")
...
png_spent_initial_year <- png_initial_year |> 
  ...
  mutate(apropiacion_vigente_pct = apropiacion_vigente / sum(apropiacion_vigente),
         label = str_glue("{number(apropiacion_vigente_pct, scale = 1e2, accuracy = 0.01, suffix = '%')}
...
png_spent_initial_year |> 
  ...
  labs(fill = NULL,
       ...
       subtitle = str_glue("Total national budget: {number(sum(png_spent_initial_year$apropiacion_vigente), big.mark = ',')} COP
...
png_spent_middle_year |> 
  ...
  labs(fill = NULL,
       ...
       subtitle = str_glue("Total national budget: {number(sum(png_spent_middle_year$apropiacion_vigente), big.mark = ',')} COP
                           Observation: B means billions in long scale (10^12)"),
       caption = str_glue("Portal de transparencia económica - https://www.pte.gov.co/
                          Last update date: {last_update_tax_revenue}")) +
  theme(
...
png_spent_last_year |> 
  ...
  labs(fill = NULL,
       ...
       subtitle = str_glue("Total national budget: {number(sum(png_spent_last_year$apropiacion_vigente), big.mark = ',')} COP
                           Observation: B means billions in long scale (10^12)"),
       caption = str_glue("Portal de transparencia económica - https://www.pte.gov.co/
                          Last update date: {last_update_tax_revenue}")) +
  theme(
...
pgn_education_clean <- pgn_education |> 
  mutate(apropiacion_vigente_pct = apropiacion_vigente / sum(apropiacion_vigente)) |> 
  mutate(apropiacion_vigente_pct_label = number(x = apropiacion_vigente_pct, 
                                                scale = 100, 
                                                accuracy = 0.01, 
                                                suffix = " %")) |> 
