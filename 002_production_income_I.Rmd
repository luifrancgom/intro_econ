...
- Output/Production approach (Colombia, Period: 2021-Q1)

```{r}
gdp_agg_value <- read_excel("000_data/002_anexos_produccion_corrientes_II_2021.xlsx", 
                            sheet = 2, 
                            range = "C15:BQ29", 
                            col_names = FALSE)

gdp_agg_value_table <- gdp_agg_value %>% 
  # Data for 2021-Q1 is not available for the income approach
  select(1, ncol(gdp_agg_value) - 1) %>% 
  rename(concept = `...1`, value_2017_1 = `...66`) %>% 
  mutate(value_2017_1 = round(value_2017_1, digits = 0))
...
gdp_agg_value_table_footnote %>% 
  ...
  footnote(general       = c('Source: DANE - Cuentas Nacionales Trimestrales - Producto Interno Bruto desde el enfoque de la producción a precios corrientes - Cuadro 1 Datos originales', 'Last update: 2021-08-17'), 
           general_title = "",
...
- Income approach (Colombia, Period: 2021-Q1)

```{r}
gdp_income <- read_excel("000_data/002_anexos_ingreso_corrientes_I_2021.xlsx",
                            sheet = 1,
                            range = "C14:X42",
                            col_names = FALSE)
...
gdp_income_table <- gdp_income %>%
  ...
  rename(concept = `...1`, value_2017_1 = `...22`) %>%
  slice(1:3, 16, 29) %>%
  mutate(value_2017_1 = round(value_2017_1, digits = 0))
...
gdp_income_table_footnote %>%
  ...
  footnote(general       = c('Source: DANE - Cuentas Nacionales Trimestrales - PIB por Ingreso - S1 Total Economía', 'Last update: 2021-06-30'),
           general_title = "",
...
- Expenditure/Final demand approach (Colombia, Period: 2021-Q1)

```{r}
gdp_expenditure <- read_excel("000_data/002_anexos_gasto_corrientes_II_2021.xlsx",
                            sheet = 2,
                            range = "B15:BP21",
                            col_names = FALSE)
...
gdp_expenditure_table <- gdp_expenditure %>%
  ...
  mutate(concept = replace(concept,
         ...
         value_2017_1 = round(value_2017_1, digits = 0))
...
gdp_expenditure_table_footnote %>%
  ...
  footnote(general       = linebreak(c('Source: DANE - Cuentas Nacionales Trimestrales - Producto Interno Bruto desde el enfoque del gasto a precios corrientes - Cuadro 1 Datos originales', 'Last update: 2021-08-17')),
           general_title = "",
