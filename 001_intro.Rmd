...
- This presentation is based on [@cardenas_santamaria_introduccion_2013, Chapter 1]
...
schools_bilin_tbl <- schools_tbl %>% 
  count(SECTOR_NOMBRE, ES_BILINGUE, TIPOBIL_NOMBRE) %>% 
...
schools_bilin_tbl %>% 
  ...
  labs(x = NULL,
       ...
       caption = str_glue('Source: DANE - Educación Formal - 2019 - Carátula única sede educativa
...
```{r out.width = "80%"}
manufac_tbl <- read_delim(file = "000_data/001_encuesta_anual_manufacturera_2019.csv", 
                            delim = ";", 
                            locale = locale(decimal_mark = ",")) %>% 
  janitor::clean_names() %>%
...
```{r out.width = "85%"}
egg_jan_tbl <- read_excel(path = "000_data/001_series_historicas_precios_mayoristas_2021_jan_july.xlsx", 
                          sheet = 2, 
                          range = "A7:E4608") %>% 
  janitor::clean_names() %>%
...
egg_july_tbl <- read_excel(path = "000_data/001_series_historicas_precios_mayoristas_2021_jan_july.xlsx", 
                           sheet = 8, 
                           range = "A7:E4557") %>% 
  janitor::clean_names() %>%
  select(fecha, producto:precio_promedio_por_kilogramo) %>%
