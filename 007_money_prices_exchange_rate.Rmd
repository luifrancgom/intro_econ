...
```{r out.width="90%"}
              ...
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de cambio y sector externo > Tasas de cambio nominales
              # Tasa Representativa del Mercado (TRM) > Diaria > DESCARGAR >
              # Descargar datos en CSV
cop_usd_trm <- read_csv2(file   = "000_data/007_trm_cop_usd_consolidado_series.csv", 
         locale = locale(decimal_mark = ".")) %>% 
    set_names(nm = c("date", "trm")) %>%
    # https://www.banrep.gov.co/es > Estadísticas económicas > Sector externo, tasas de cambio y derivados >
...
- In the link **https://www.banrep.gov.co/es/estadisticas/tasas-interes-politica-monetaria** on __2023-01-10__ it was pointed out that:
...
```
    ...
    + Also the *"tasa de intervención de política monetaria"* on __2023-01-10__ was `r scales::percent(i_t, accuracy = 0.01)`
...
```{r out.width="90%"}
              # https://www.banrep.gov.co/es > Estadísticas económicas >
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Tasas de interés
              # Tasa de política monetaria > Diaria > DESCARGAR >
              # Descargar datos en CSV
pir_daily <- read_csv2(file = '000_data/007_tip_serie_historica_diaria.csv') %>% 
...
```{r out.width="90%"}
              ...
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Tasas de interés
              # Tasa interbancaria (TIB) > Diaria > DESCARGAR >
              # Descargar datos en CSV
tib_daily <- read_csv2(file  = '000_data/007_tib_serie_historica_diaria.csv') %>% 
  set_names(nm = c('date', 'tib'))
              # https://www.banrep.gov.co/es > Estadísticas económicas >
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Tasas de interés
              # Indicador Bancario de Referencia (IBR) overnight, efectiva > Diaria > DESCARGAR >
              # Descargar datos en CSV
ibr_daily <- read_csv2(file = "000_data/007_ibr_plazo_overnight_efectiva_diaria.csv") %>% 
...
```{r out.width="90%"}
              ...
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Tasas de interés
              # Tasa de Certificado de Depósito a Término (CDT) a 90 días > Diaria > DESCARGAR >
              # Descargar datos en CSV
cdt_90 <- read_csv2(file  = '000_data/007_cdt_90_diaria.csv') %>% 
  set_names(nm = c('date', 'cdt_90')) 
              # https://www.banrep.gov.co/es > Estadísticas económicas >
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Tasas de interés
              # Tasa de Certificado de Depósito a Término (CDT) a 180 días > Diaria > DESCARGAR >
              # Descargar datos en CSV
cdt_180 <- read_csv2(file = "000_data/007_cdt_180_diaria.csv") %>% 
  set_names(nm = c('date', 'cdt_180'))
              # https://www.banrep.gov.co/es > Estadísticas económicas >
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Tasas de interés
              # Tasa de Certificado de Depósito a Término (CDT) a 360 días > Diaria > DESCARGAR >
              # Descargar datos en CSV
cdt_360 <- read_csv2(file = "000_data/007_cdt_360_diaria.csv") %>% 
