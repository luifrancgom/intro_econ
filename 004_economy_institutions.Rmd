...
- This presentation is based on [@cardenas_santamaria_introduccion_2013, Chapter 4]
...
initial_year <- 1996
last_year <- 2019
middle_year <- ceiling((2019 - 1996) / 2) + initial_year
...
```{r out.width="90%"}
homicide_1990_2018 <- read_csv("000_data/004_homicide_table_full_data.csv") %>% 
  janitor::clean_names() %>% 
  filter(year %in% c(1990, 2018),
         indicator == "Homicide rate") %>% 
  pivot_wider(id_cols = c(region:indicator), 
              names_from = year, values_from = value) %>% 
  filter(!(is.na(`2018`) | is.na(`1990`)))

homicide_1990_2018 %>% 
  ggplot(aes(x = `1990`, y = `2018`)) + 
  geom_abline(slope = 1, color = 'black') +
  geom_point(aes(fill = region),
             shape = 21, 
             size = 4) + 
  geom_label_repel(data = homicide_1990_2018 %>% 
  ...
  scale_fill_tq() +
  labs(fill = NULL, 
       title = "Intentional homicide rate per 100,000 population: 1990 vs 2018",
       subtitle = str_glue("Number of countries: {nrow(homicide_1990_2018)}"),
       caption = str_glue("Source: United Nations Office on Drugs and Crime - DATAUNODC
...
```{r out.width="90%"}
kidnapping_2004_2018 <- read_csv("000_data/004_kidnapping_table_data.csv") %>% 
  janitor::clean_names() %>% 
  filter(year %in% c(2004, 2018),
         measure_names == "Rate") %>% 
  pivot_wider(id_cols = c(region:country), 
              names_from = year, values_from = measure_values) %>% 
  filter(!(is.na(`2004`) | is.na(`2018`)))

kidnapping_2004_2018 %>% 
  ggplot(aes(x = `2004`, y = `2018`)) + 
  geom_abline(slope = 1, color = 'black') +
  geom_point(aes(fill = region),
             shape = 21, size = 4) + 
  geom_label_repel(data = kidnapping_2004_2018 %>% 
                     filter(country == "Colombia"),
                   aes(label = country), 
                   seed = 1234,                    
                   box.padding = 17) + 
  scale_fill_tq() +
  labs(fill = NULL, 
       title = "Kidnapping, police-recorded offences, per 100,000 population: 2004 vs 2018",
       subtitle = str_glue("Number of countries: {nrow(kidnapping_2004_2018)}"),
       caption = str_glue("Source: United Nations Office on Drugs and Crime - DATAUNODC
                          Last update: 2018-10-12")) + 
  theme(panel.border    = element_rect(fill = NA, color = "black"),
        plot.background   = element_rect(fill = "#f3fcfc"),
...
```{r out.width="90%"}
theft_2004_2018 <- read_csv("000_data/004_theft_full_data.csv") %>% 
  janitor::clean_names() %>%
  filter(year %in% c(2004, 2018)) %>% 
  pivot_wider(id_cols = c(country, 
                          population_type:sub_region), 
              names_from = year, values_from = rate) %>% 
  filter(!(is.na(`2004`) | is.na(`2018`)))
...
theft_2004_2018 %>% 
  ...
  geom_point(aes(fill = region),
             shape = 21, size = 4) + 
  geom_label_repel(data = theft_2004_2018 %>% 
                     filter(country == "Colombia"),
                   aes(label = country), 
                   seed = 1234,                    
                   box.padding = 10) + 
  scale_fill_tq() +
  labs(fill = NULL, 
       title = "Theft, police-recorded offences, per 100,000 population: 2004 vs 2018",
       subtitle = str_glue("Number of countries: {nrow(theft_2004_2018)}"),
       caption = str_glue("Source: United Nations Office on Drugs and Crime - DATAUNODC
                          Last update: 2018-10-12")) + 
  theme(panel.border    = element_rect(fill = NA, color = "black"),
        plot.background   = element_rect(fill = "#f3fcfc"),
