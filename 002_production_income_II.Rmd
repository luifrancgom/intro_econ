...
- This presentation is based on [@cardenas_santamaria_introduccion_2013, Chapter 2]
...
```{r}
gdp_expenditure_annual <- readxl::read_excel("000_data/002_agregados_macroeconomicos_cuentas_nal_anuales_2005-2019p-2020pr.xlsx", 
                                             sheet = 3, 
                                             range = "E129:U147", 
                                             col_names = FALSE, 
                                             trim_ws = TRUE)
...
gdp_expenditure_annual_table <- gdp_expenditure_annual %>% 
  ...
  slice(1:2, 5, 7, 16:19) %>% 
  rename(concept = `...1`, value_2015 = `...12`, value_2019 = `...16`, value_2020 = `...17`) %>% 
  mutate(concept = case_when(
    ...
    value_2019 = round(as.numeric(value_2019), digits = 0),
...
names(gdp_expenditure_annual_table_footnote)[3] <- "2019"
names(gdp_expenditure_annual_table_footnote)[4] <- "2020"
...
gdp_expenditure_annual_table_footnote %>% 
  ...
  footnote(general       = c('Source: DANE - Cuentas Nacionales Anuales - Producto Interno Bruto (PIB) - Series encadenadas de volumen con año de referencia 2015', 'Last update: 2021-05-14'),
           general_title = "",
...
```{r}
gdp_expenditure_annual_current <- readxl::read_excel("000_data/002_agregados_macroeconomicos_cuentas_nal_anuales_2005-2019p-2020pr.xlsx",
                                             sheet = 2,
                                             range = "E119:U136",
                                             col_names = FALSE,
                                             trim_ws = TRUE)
...
names(gdp_expenditure_annual_current_table_footnote)[3] <- "2019"
names(gdp_expenditure_annual_current_table_footnote)[4] <- "2020"
...
gdp_expenditure_annual_current_table_footnote %>%
  ...
  row_spec(row = 6, background = '#FF7F00') %>%
  row_spec(row = 7, background = '#e31a1c') %>%
  footnote(general       = c('Source: DANE - Cuentas Nacionales Anuales - Producto Interno Bruto (PIB) - Precios corrientes', 'Last update: 2021-05-14'),
           general_title = "",
           alphabet = c('Provisional data',
                        'Preliminary data',
                        'Instituciones sin fines de lucro que sirven a los hogares'),
...
gdp_sector_annual_current_2014_2020 <- read_excel("000_data/002_agregados_macroeconomicos_cuentas_nal_anuales_2005-2019p-2020pr.xlsx", 
                                                  sheet = 2, 
                                                  range = "B15:U114",
                                                  col_names = c("id", "id_25", "id_61", "sector", as.character(2005:2020))) %>% 
  mutate(id = replace(id, sector == "Impuestos menos subvenciones sobre los productos", "Impuestos - Subsidios")) %>% 
  filter(!is.na(id)) %>% 
  select(-c("id_25", "id_61")) %>% 
  pivot_longer(cols = `2005`:`2020`, 
               names_to = "year",
               values_to = "value") %>% 
  mutate(year = as.integer(year)) %>% 
  filter(year %in% 2014:2020)

gdp_sector_annual_current_1975_2020 <- gdp_sector_annual_current_1975_2013 %>%
  bind_rows(gdp_sector_annual_current_2014_2020) %>% 
  group_by(year) %>% 
...
gdp_sector_annual_current_year <- gdp_sector_annual_current_1975_2020 %>% 
  # Specify the year
  filter(year %in% 1975)
