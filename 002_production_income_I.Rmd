...
- Output/Production approach (Colombia, Period: 2021-Q4)
...
# Información técnica
gdp_agg_value <- read_excel("000_data/002_anexos_produccion_corrientes_I_2022.xlsx", 
                            sheet = 2, 
                            range = "C15:BT29", 
                            col_names = FALSE)
...
gdp_agg_value_table <- gdp_agg_value %>% 
  ...
  rename(concept = `...1`, value_2021_4 = `...69`) %>% 
  mutate(value_2021_4 = round(value_2021_4, digits = 0))
...
gdp_agg_value_table_footnote %>% 
  ...
  footnote(general       = c('Source: DANE - Cuentas Nacionales Trimestrales - Producto Interno Bruto desde el enfoque de la producción a precios corrientes - Cuadro 1 Datos originales', 'Last update: 2022-06-13'), 
           general_title = "",
...
- Income approach (Colombia, Period: 2021-Q4)

```{r}
gdp_income <- read_excel("000_data/002_anexos_ingreso_corrientes_IV_2021.xlsx",
                         sheet = 1,
                         range = "C14:AB42",
                         col_names = FALSE)
...
gdp_income_table <- gdp_income %>%
  ...
  rename(concept = `...1`, value_2021_3 = `...26`) %>%
  slice(1:3, 16, 29) %>%
  mutate(value_2021_3 = round(value_2021_3, digits = 0))
...
gdp_income_table_footnote %>%
  ...
  footnote(general       = c('Source: DANE - Cuentas Nacionales Trimestrales - PIB por Ingreso - S1 Total Economía', 'Last update: 2022-06-13'),
           general_title = "",
...
- Expenditure/Final demand approach (Colombia, Period: 2021-Q4)

```{r}
gdp_expenditure <- read_excel("000_data/002_anexos_gasto_corrientes_I_2022.xlsx",
                              sheet = 2,
                              range = "B15:BS21",
                              col_names = FALSE)
...
gdp_expenditure_table <- gdp_expenditure %>%
  ...
  mutate(concept = replace(concept,
         ...
         value_2021_4 = round(value_2021_4, digits = 0))
...
gdp_expenditure_table_footnote %>%
  ...
  footnote(general       = linebreak(c('Source: DANE - Cuentas Nacionales Trimestrales - Producto Interno Bruto desde el enfoque del gasto a precios corrientes - Cuadro 1 Datos originales', 'Last update: 2022-06-14')),
           general_title = "",
