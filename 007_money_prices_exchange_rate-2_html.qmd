...
ibr_daily_clean <- ibr_daily |>
  ...
  select(`Fecha (dd/mm/aaaa)`, `Tasa efectiva (%)`) |>
...
cdt_90_180_360_clean <- cdt_90_180_360 |>
  # Check column names if they change
  select(`Fecha (dd/mm/aaaa)`, Cuenta, `Tasa efectiva anual (%)`) |> 
  # Check column names if they change
  pivot_wider(id_cols = `Fecha (dd/mm/aaaa)`, 
              names_from = Cuenta, 
              values_from = `Tasa efectiva anual (%)`) |> 
