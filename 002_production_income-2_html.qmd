...
# Import
gdp_expenditure_annual <- read_excel(
...
gdp_expenditure_annual_table <- gdp_expenditure_annual |>
  select(
    1,
    12,
    ncol(gdp_expenditure_annual) - 1,
    ncol(gdp_expenditure_annual)
  ) |>
  slice(1:4, 6, 15:18) |>
  ...
  mutate(
    concept = case_when(
      concept ==
        "Gasto de consumo final individual de los hogares; gasto de consumo final de las ISFLH1" ~ "Gasto de consumo final individual de los hogares y las ISFLH",
      TRUE ~ concept
    )
  )
...
# Import
gdp_expenditure_annual_current <- read_excel(
  "000_data/002_agregados_macroeconomicos_cuentas_nal_anuales_2005_2023p_2024pr.xlsx",
  sheet = 2,
  range = "E119:Y135",
  col_names = FALSE,
...
gdp_expenditure_annual_current_table <- gdp_expenditure_annual_current |>
  select(
    1,
    12,
    ncol(gdp_expenditure_annual_current) - 1,
    ncol(gdp_expenditure_annual_current)
  ) |>
  slice(1:4, 6, 15:17) |>
...
gdp_expenditure_annual_table_clean <- gdp_expenditure_annual_current_table |>
  ...
  tab_source_note(
    source_note = str_glue(
      "Last update: {last_update_gdp_expenditure_annual_current}"
    )
  ) |>
  tab_footnote(
    footnote = "Instituciones sin fines de lucro que sirven a los hogares",
...
<!-- Check out 000_gifs/002_gdp_sector_plot_anim.R -->
...
gdp_agg_demand_pct_clean |>
  ...
  labs(
    ...
    y = "Percent",
    subtitle = str_glue(
      "C (% GDP) code WDI: NE.CON.PRVT.CN, G (% GDP) code WDI: NE.CON.GOVT.CN
...
- Primer corte 30% \> Learning Resources \> Links of interest \> Banco de la República de Colombia - Cátedras de Macroeconomía y Banca Central - Indicadores de actividad económica
