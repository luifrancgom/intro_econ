...
# Helpers
last_update_bop_banrep <- ymd("2025-03-31")
...
# Import
bop_col_current_account <- imf_dataset(
  database_id = "BOP",
  freq = "A",
  ref_area = "CO",
  # Current Account
...
bop_col_financial_account_table_2 |>
  ...
  tab_style(
    style = cell_fill(color = umng_palette[11]),
    locations = cells_body(rows = c(2, 4, 6, 7))
  ) |>
  tab_footnote(
    footnote = "Provisional data (Millions of current USD)",
    locations = cells_column_labels(columns = c(
...
```{r}
# Import
bop_col_financial_account <- imf_dataset(
  database_id = "BOP",
  freq = "A",
  ref_area = "CO",
  # Financial Account
...
[**imfr**](https://github.com/christophergandrud/imfr) and [**tinytex**](https://CRAN.R-project.org/package=tinytex) for allowing me to access these tools without paying for a license
