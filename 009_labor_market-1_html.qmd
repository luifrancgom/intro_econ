...
```{r}
# Import ----
employment_unemployment <- wb_data(country = c('COL'),
...
employment_unemployment_clean |> 
  ...
  facet_wrap(facet    = vars(indicator), 
             ...
             nrow     = 2, 
             scales   = "free_y") +
  scale_x_continuous(breaks = c(1991, 1999, 2010, 
                                max(employment_unemployment_clean$date))) +
  scale_y_continuous(labels = label_number(accuracy = 1,
                                           suffix = "%")) +
  coord_cartesian(xlim = c(min(employment_unemployment_clean$date), 
                           max(employment_unemployment_clean$date))) +
  labs(x        = 'Year',
       y        = 'Percent',
...
econ_growth_unemployment_long <- wb_data(indicator = c("NY.GDP.PCAP.KD.ZG",
...
group_ingreso_nivel_educativo_categories |> 
  ...
  labs(x = "Number of employees in thousands (K)",
       ...
       caption = str_glue("Gran Encuesta Integrada de Hogares - GEIH - {year}-{month}")) +
  theme(
    panel.border = element_rect(
      fill = NA,
      color = "black"
