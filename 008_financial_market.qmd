...
credit_private %>%
  ...
  geom_point(color = "black",
             ...
             shape = 21,
             show.legend = FALSE) +
  geom_label_repel(data = filter(credit_private, iso3c %in% c("COL", "MAC", "SLE")),
                   aes(label = country),
                   seed = 1234,
                   size = 3,
...
```{r out.width="90%"}
              # https://www.banrep.gov.co/es > Estadísticas económicas >
              # !NUEVO! Estadísticas Banrep > CATÁLOGO DE SERIES > 
              # Tasas de interés y sector financiero > Mercado accionario
              # Índice COLCAP 
              # > Diaria > DESCARGAR >
              # Descargar datos en CSV
...
bank_table %>%
  ...
  footnote(general       = c('Source: Superintendencia Financiera de Colombia', 'Last update: 2023-07-20'),
           general_title = "",
           threeparttable = TRUE)
```
