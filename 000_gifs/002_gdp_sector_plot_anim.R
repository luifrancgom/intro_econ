...
# Import ----
gdp_sector_annual_current_1975_2013 <- read_excel(
  "000_data/002_anexo_retropolacion_base_2015.xlsx",
  sheet = 2,
  range = "B13:AP26",
...
gdp_sector_annual_current_2014_2024 <- read_excel(
  "000_data/002_agregados_macroeconomicos_cuentas_nal_anuales_2005_2023p_2024pr.xlsx",
  sheet = 2,
  range = "B14:Y114",
  col_names = c("id", "id_25", "id_61", "sector", as.character(2005:2024))
)
...
gdp_sector_annual_current_1975_2013_clean <- gdp_sector_annual_current_1975_2013 |>
  ...
  slice(-13) |>
  mutate(
    id = replace(id, is.na(id), "Impuestos - Subsidios"),
    # There is a typo in the file: +S must be + S
    id = replace(id, id == "R +S + T", "R + S + T")
  ) |>
  pivot_longer(cols = `1975`:`2013`, names_to = "year", values_to = "value") |>
...
gdp_sector_annual_current_2014_2024_clean <- gdp_sector_annual_current_2014_2024 |>
  ...
  pivot_longer(cols = `2005`:`2024`, names_to = "year", values_to = "value") |>
  mutate(year = as.integer(year)) |>
...
gdp_sector_plot_static <- gdp_sector_annual_current_1975_2024_clean |>
  ...
  geom_treemap(color = "black", show.legend = FALSE) +
  geom_treemap_text(place = "centre") +
  scale_fill_manual(values = umng_palette) +
  labs(
    title = "Year: {frame_time}",
    caption = str_glue(
      "Source: DANE - Cuentas Nacionales Anuales - Retropolación Base 2015 - PIB enfoque de la producción y del gasto a precios corrientes
...
gdp_sector_plot_anim <- animate(
  gdp_sector_plot_static,
  nframes = 400,
  fps = 5,
  width = 900,
  height = 600
...
# Export ----
anim_save(
  filename = "000_gifs/002_gdp_sector_plot_anim.gif",
