...
- **Current account**: "shows flows of goods, services, primary income, and secondary income between residents and nonresidents" [@international_monetary_fund_balance_2009, paragraph 10.1]

    + **Goods and services account**: "shows transactions in items that are outcomes of production activities" [@international_monetary_fund_balance_2009, paragraph 2.14]
...
bop_col <- read_excel(path = "000_data/005_1_1_1_bp_resumen_iqy.xlsx", 
           ...
           range = "A11:W53") %>% 
  select(1, 20:22) %>% 
  set_names(nm = c("Account", as.character(2018:2020)))
...
bop_col_current_account_table_footnote %>%
  ...
  footnote(general       = c('Source: Banco de la República - Colombia', 'Last update: 2021-09-15'),
           general_title = "",
           number = 'Methodology: Sixth version of the Balance of Payments Manual',
           alphabet = c('Provisional data (Millions of current USD)', 'Preliminary data (Millions of current USD)'), 
           threeparttable = TRUE)
```
...
bop_col_2000_2020 <- imf_data(database_id = "BOP",
                              # Current Account
...
```{r out.width="90%"}
bop_col_2000_2020_longer <- bop_col_2000_2020 %>%
  as_tibble() %>%
...
names(bop_col_financial_account_table)[2] <-  paste0(names(bop_col_financial_account_table)[2],
                                                            footnote_marker_alphabet(1, "latex"))

bop_col_financial_account_table_footnote <- bop_col_financial_account_table

names(bop_col_financial_account_table)[3:4] <-  paste0(names(bop_col_financial_account_table)[3:4],
                                                              footnote_marker_alphabet(2, "latex"))
